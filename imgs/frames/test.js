...
var rates = document.getElementsByName('answer-1');
var rates_score;
for(var i = 0; i < rates.length; i++){
	if(rates[i].checked == true){
	rates_score = rates[i].value;
	}
}	
...
/*	    var methods_1 = document.getElementById("approvalQuiz").q1_method,questOne;
		...
		for (var s = 0; s < methods_11.length; s++) {
			if(methods_11[s].checked == true){
				...
				var score =  (questOnePts + questTwoPts + questThreePts + questFourPts + questFivePts + questSixPts + questSevenPts + questEightPts + questNinePts + questTenPts + questElevenPts + questTwelvePts + 50).toFixed(2);
				...
				document.getElementById("ttl-estimate").value = "$" + approval.toLocaleString('en');
